...
export default function DashboardPage() {
  ...
  const trend = dashboard?.income_trends ?? [];
    ...
    <div style={{ padding: "1rem", width: "100%" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
        <Card style={{ flex: 2, background: "#fff" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
            <div>
              ...
              <strong style={{ fontSize: "1.6rem", fontWeight: 600, color: TEXT_MAIN }}>{dashboard?.score ?? 0}</strong>
            </div>
            <Badge tone={(dashboard?.score ?? 0) >= 680 ? "good" : "warn"}>{dashboard?.grade ?? "—"}</Badge>
          </div>
          <p style={{ fontSize: "0.8rem", color: TEXT_MUTED, marginBottom: "1rem" }}>{dashboard?.financial_health ?? ""}</p>
        ...
        <div style={{ flex: 1, background: "#fff", borderRadius: 12, padding: "1rem", border: `1px solid ${BORDER}`, display: "flex", alignItems: "center" }}>
          <div>
            ...
            <strong style={{ fontSize: "1.6rem", fontWeight: 600, color: TEXT_MAIN }}>{riskAverage}</strong>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
        <Card style={{ background: "#fff", padding: "0.8rem" }}>
          ...
          <strong style={{ fontSize: "1.4rem", fontWeight: 600, color: TEXT_MAIN }}>{cases.filter(c => c.status !== "RESOLVED").length}</strong>
        </Card>
        <Card style={{ background: "#fff", padding: "0.8rem" }}>
          ...
          <strong style={{ fontSize: "1.4rem", fontWeight: 600, color: TEXT_MAIN }}>{dashboard?.flagged_anomalies.length ?? 0}</strong>
        </Card>
        <Card style={{ background: "#fff", padding: "0.8rem" }}>
